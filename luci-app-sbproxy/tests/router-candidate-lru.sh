...
TEST_ROOT='/tmp/sbproxy-adaptive-candidate-lru-test'
...
i=0
while [ "$i" -lt 256 ]; do
	printf 'ERROR test: open connection to candidate-%03d.example:443 using outbound/direct[sbproxy-adaptive-final-direct-out]: failed\n' \
		"$i" >>"$CORE_LOG"
	i=$((i + 1))
done
...
printf '%s\n' \
	'ERROR refresh: open connection to candidate-000.example:443 using outbound/direct[sbproxy-adaptive-final-direct-out]: failed' \
	'ERROR insert: open connection to candidate-256.example:443 using outbound/direct[sbproxy-adaptive-final-direct-out]: failed' \
	>>"$CORE_LOG"
