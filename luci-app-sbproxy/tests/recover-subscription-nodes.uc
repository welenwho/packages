...
const configDir = getenv('SBPROXY_UCI_CONFIG_DIR');
if (!configDir)
	die('SBPROXY_UCI_CONFIG_DIR is required\n');

const uci = cursor(configDir);
const current = 'sbproxy';
const backup = 'sbproxy_backup';
uci.load(current);
uci.load(backup);
...
let retargeted = 0;
const brokenOutbound = getenv('SBPROXY_RECOVERY_BROKEN_OUTBOUND');
const fallbackOutbound = getenv('SBPROXY_RECOVERY_FALLBACK_OUTBOUND');
