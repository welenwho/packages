...
MODULE_DIR="${2:-/tmp}"
TEST_ROOT='/tmp/sbproxy-adaptive-ip-test'
...
cat >"$TEST_ROOT/sing-box.json" <<-EOF
{
  ...
  "inbounds": [
    { "type": "socks", "tag": "sbproxy-adaptive-direct-probe-in", "listen": "127.0.0.1", "listen_port": $DIRECT_PORT },
    { "type": "socks", "tag": "sbproxy-adaptive-proxy-probe-in", "listen": "127.0.0.1", "listen_port": $PROXY_PORT }
  ],
  "outbounds": [
    { "type": "direct", "tag": "direct-out", "routing_mark": 100 },
    { "type": "direct", "tag": "sbproxy-adaptive-out", "routing_mark": 100 }
  ],
  "route": {
    "rules": [
      { "inbound": "sbproxy-adaptive-direct-probe-in", "action": "route", "outbound": "direct-out", "override_address": "192.0.2.1", "override_port": 443 },
      { "inbound": "sbproxy-adaptive-proxy-probe-in", "action": "route", "outbound": "sbproxy-adaptive-out" }
    ],
    "final": "direct-out"
  },
...
sleep 6
printf '%s\n' \
	'ERROR final-ip: open connection to 223.5.5.5:443 using outbound/direct[sbproxy-adaptive-final-direct-out]: failed' \
