...
function parseShareLink(uri, features) {
	...
	if (uri[0] && uri[1]) {
			...
			try {
				...
				try {
					...
					if (suri.length <= 2) {
						if (suri.length === 2)
							slabel = '#' + suri[1];
						uri[1] = sb.decodeBase64Str(suri[0]) + slabel;
					}
				} catch(e) { }
				...
				if (url.username && url.password) {
					...
					userinfo = sb.decodeBase64Str(decodeURIComponent(url.username)).split(':');
					if (userinfo.length > 1)
						userinfo = [userinfo[0], userinfo.slice(1).join(':')]
				}

				if (!sb.shadowsocks_encrypt_methods.includes(userinfo[0]))
					return null;
		...
		case 'vmess':
			...
			uri = JSON.parse(sb.decodeBase64Str(uri[1]));
...
function renderNodeSettings(section, data, features, main_node, routing_mode, node_latency_row_state) {
	...
	s.max_cols = 7;
	s.modaltitle = L.bind(sb.loadModalTitle, this, _('Node'), _('Add a node'), data[0]);
	s.sectiontitle = L.bind(sb.loadDefaultLabel, this, data[0]);
	...
	o.load = L.bind(sb.loadDefaultLabel, this, data[0]);
	o.validate = L.bind(sb.validateUniqueValue, this, data[0], 'node', 'label');
	o.rmempty = false;
	o.modalonly = true;
	...
	o.depends('type', 'hysteria2');
	o.validate = sb.validatePortRange;
	o.modalonly = true;
	...
	o = s.option(form.Value, 'hysteria_hop_interval_max', _('Maximum hop interval'),
		_('Maximum randomized port hopping interval in seconds. Leave empty to use the regular interval.'));
	o.datatype = 'uinteger';
	o.depends({'type': 'hysteria2', 'hysteria_hopping_port': /[\s\S]/});
	o.modalonly = true;
	...
	for (let i of sb.shadowsocks_encrypt_methods)
		o.value(i);
	/* Stream ciphers */
	o.value('aes-128-ctr');
	o.value('aes-192-ctr');
	...
	o.onchange = function(ev, section_id, value) {
		let desc = this.map.findElement('id', 'cbid.sbproxy.%s.transport'.format(section_id)).nextElementSibling;
		if (value === 'http')
		...
		if ((value === 'http' && tls.checked) || (value === 'grpc' && !features.with_grpc)) {
			this.map.findElement('id', 'cbid.sbproxy.%s.http_idle_timeout'.format(section_id)).nextElementSibling.innerHTML =
			...
			this.map.findElement('id', 'cbid.sbproxy.%s.http_ping_timeout'.format(section_id)).nextElementSibling.innerHTML =
	...
	o.password = true;
	o.depends('type', 'wireguard');
	o.validate = L.bind(sb.validateBase64Key, this, 44);
	o.rmempty = false;
	o.modalonly = true;
	...
	o.value('/etc/sbproxy/certs/client_ca.pem');
	...
	o.depends({'tls_self_sign': '1', 'tls_cert_path': '/etc/sbproxy/certs/client_ca.pem'});
	o.onclick = L.bind(sb.uploadCertificate, this, _('certificate'), 'client_ca');
	o.modalonly = true;
	...
	o.value('/etc/sbproxy/certs/client_ech_conf.pem');
	o.depends('tls_ech', '1');
	o.modalonly = true;
	...
	o.inputstyle = 'action';
	o.inputtitle = _('Upload...');
	o.depends({'tls_ech': '1', 'tls_ech_config_path': '/etc/sbproxy/certs/client_ech_conf.pem'});
	o.onclick = L.bind(sb.uploadCertificate, this, _('ECH config'), 'client_ech_conf');
	o.modalonly = true;
	...
	if (features.with_utls) {
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				let tls_reality = this.map.findElement('id', 'cbid.sbproxy.%s.tls_reality'.format(section_id)).firstElementChild;
	...
	o = s.option(form.Value, 'connect_timeout', _('Connection timeout'),
		_('Maximum outbound connection establishment time in seconds. Leave empty for the core default.'));
	o.datatype = 'uinteger';
	o.modalonly = true;

	o = s.option(form.Flag, 'disable_tcp_keep_alive', _('Disable TCP keepalive'));
	o.depends('type', /^(anytls|direct|http|shadowsocks|shadowtls|socks|ssh|trojan|vless|vmess)$/);
	o.modalonly = true;

	o = s.option(form.Value, 'tcp_keep_alive', _('TCP keepalive idle time'),
		_('Idle time in seconds before TCP keepalive probes begin. Leave empty for the core default.'));
	o.datatype = 'uinteger';
	o.depends({'type': /^(anytls|direct|http|shadowsocks|shadowtls|socks|ssh|trojan|vless|vmess)$/, 'disable_tcp_keep_alive': '0'});
	o.modalonly = true;

	o = s.option(form.Value, 'tcp_keep_alive_interval', _('TCP keepalive interval'),
		_('Interval in seconds between TCP keepalive probes. Leave empty for the core default.'));
	o.datatype = 'uinteger';
	o.depends({'type': /^(anytls|direct|http|shadowsocks|shadowtls|socks|ssh|trojan|vless|vmess)$/, 'disable_tcp_keep_alive': '0'});
	o.modalonly = true;
...
return view.extend({
	...
	render(data) {
		...
		for (let suburl of (uci.get(data[0], 'subscription', 'subscription_url') || [])) {
			const urlhash = sb.calcStringMD5(suburl.replace(/#.*$/, ''));
		...
		m = new form.Map('sbproxy', _('Edit Nodes'));
		const saveMap = m.save;
		m.save = function(cb, silent) {
			return saveMap.call(this, () => Promise.resolve(
				typeof cb === 'function' ? cb() : null
			).then(() => sb.reconcileUrltestNodes(data[0])), silent);
		};

		s = m.section(form.NamedSection, 'subscription', 'sbproxy');
		...
		o.onclick = function() {
			return fs.exec('/etc/sbproxy/scripts/update_subscriptions.sh').then((res) => {
				if (res.code !== 0) {
		...
		return m.render().then((el) => {
			...
			let bulk_button = E('button', {
				'class': 'cbi-button cbi-button-action sbp-bulk-latency-test',
			...
			let attachBulkButton = () => {
				...
				if (!actions.querySelector('.sbp-bulk-latency-test'))
