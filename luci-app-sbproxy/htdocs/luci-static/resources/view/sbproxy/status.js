...
const resources = [
	{
		type: 'geoip_cn',
		name: _('China IP rules (IPv4 and IPv6)')
	},
	{
		type: 'geosite_cn',
				...
				E('button', {
					...
					'click': ui.createHandlerFn(this, () => {
						return L.resolveDefault(callResUpdate(), {}).then((res) => {
							...
							if (res.apply_failed) {
								message = _('Resources were updated, but SBProxy failed to reload. Check the log for details.');
								severity = 'error';
							} else {
								switch (res.status) {
								case 0:
...
function getRuntimeLog(o, name, _option_index, section_id, _in_table) {
	...
	if (section) {
		const selected = uci.get('sbproxy', section, 'log_level') || 'warn';
		const choices = {
			trace: _('Trace'),
			debug: _('Debug'),
			info: _('Info'),
		...
		log_level_el = E('select', {
			...
			'change': ui.createHandlerFn(this, (ev) => {
				uci.set('sbproxy', section, 'log_level', ev.target.value);
	...
	let log;
	poll.add(L.bind(() => {
		return fs.read_direct(String.format('%s/%s.log', sb_dir, filename), 'text')
		.then((res) => {
			log = E('pre', { 'wrap': 'pre' }, [
...
return view.extend({
	render() {
		...
		o = s.option(form.DummyValue, '_sbproxy_logview');
		o.render = L.bind(getRuntimeLog, this, o, _('SBProxy'));
