...
const CBIGenValue = form.Value.extend({
	...
	renderWidget(/* ... */) {
		...
		(node.querySelector('.control-group') || node).appendChild(E('button', {
			...
			title: _('Generate'),
			click: ui.createHandlerFn(this, handleGenKey, this.sbp_options || this.option)
		}, [ _('Generate') ]));
...
function renderStatus(isRunning, version) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('SBProxy Server'), version, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('SBProxy Server'), version, _('NOT RUNNING'));
	...
	if (typeof option === 'object') {
		...
		switch (required_method) {
			case 'none':
				...
				break;
			case 'uuid':
				password = sb.generateRand('uuid');
				break;
			default:
				password = sb.generateRand('hex', 16);
				break;
		}
		/* AEAD */
		((length) => {
			if (length && length > 0)
				password = sb.generateRand('base64', length);
		})(sb.shadowsocks_encrypt_length[required_method]);
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('sbproxy', _('SBProxy Server'),
			_('The modern ImmortalWrt proxy platform for ARM64/AMD64. — AI Edition'));

		s = m.section(form.TypedSection);
		s.render = function() {
			poll.add(() => {
				return L.resolveDefault(sb.getServiceStatus('sing-box-s')).then((res) => {
		...
		s = m.section(form.NamedSection, 'server', 'sbproxy', _('Global Settings'));
		...
		s.modaltitle = L.bind(sb.loadModalTitle, this, _('Server'), _('Add a server'), data[0]);
		s.sectiontitle = L.bind(sb.loadDefaultLabel, this, data[0]);
		s.renderSectionAdd = L.bind(sb.renderSectionAdd, this, s);

		o = s.option(form.Value, 'label', _('Label'));
		o.load = L.bind(sb.loadDefaultLabel, this, data[0]);
		o.validate = L.bind(sb.validateUniqueValue, this, data[0], 'server', 'label');
		o.rmempty = false;
		o.modalonly = true;
		...
		o.datatype = 'port';
		o.validate = L.bind(sb.validateUniqueValue, this, data[0], 'server', 'port');
				...
				if (required_type.includes(type)) {
					if (type === 'shadowsocks') {
						...
						if (encmode === 'none')
							...
							return sb.validateBase64Key(24, section_id, value);
						else if (['2022-blake3-aes-256-gcm', '2022-blake3-chacha20-poly1305'].includes(encmode))
							return sb.validateBase64Key(44, section_id, value);
					}
		...
		o.onchange = function(ev, section_id, value) {
			...
			let tls_element = this.map.findElement('id', 'cbid.sbproxy.%s.tls'.format(section_id)).firstElementChild;
		...
		if (features.sbp_has_tcp_brutal) {
		...
		o.value('/etc/sbproxy/certs/server_publickey.pem');
		...
		o.depends({'tls': '1', 'tls_cert_path': '/etc/sbproxy/certs/server_publickey.pem'});
		o.onclick = L.bind(sb.uploadCertificate, this, _('certificate'), 'server_publickey');
		o.modalonly = true;

		o = s.option(form.Value, 'tls_key_path', _('Key path'),
			_('The server private key, in PEM format.'));
		o.value('/etc/sbproxy/certs/server_privatekey.pem');
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
		...
		o.inputstyle = 'action';
		o.inputtitle = _('Upload...');
		o.depends({'tls': '1', 'tls_key_path': '/etc/sbproxy/certs/server_privatekey.pem'});
		o.onclick = L.bind(sb.uploadCertificate, this, _('Private key'), 'server_privatekey');
		o.modalonly = true;
			...
			node.appendChild(E('div',  { 'class': 'control-group' }, [
				...
				E('button', {
					...
					click: ui.createHandlerFn(this, () => {
						this.sbp_options.params = document.getElementById(cbid).value;

						return handleGenKey.call(this, this.sbp_options);
					})
				}, [ _('Generate') ])
			]));
