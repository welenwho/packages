...
const CONFIG = 'sbproxy-adaptive';
const SECTION = 'main';
const RUN_DIR = getenv('SBPROXY_ADAPTIVE_RUN_DIR') || '/var/run/sbproxy-adaptive';
const STATUS_PATH = RUN_DIR + '/status.json';
const RULES_PATH = getenv('SBPROXY_ADAPTIVE_RULES_PATH') || RUN_DIR + '/rules.json';
const LEARNED_PATH = getenv('SBPROXY_ADAPTIVE_LEARNED_PATH') || '/etc/sbproxy/adaptive/learned.json';
const CORE_LOG_PATH = getenv('SBPROXY_ADAPTIVE_CORE_LOG_PATH') || '/var/run/sbproxy/sing-box-c.log';
...
const policy = resolveAdaptivePolicy(uci, 'sbproxy', CONFIG);
if (policy.requested && policy.mode_allowed && policy.error) {
	warn(`sbproxy-adaptive: ${policy.error}\n`);
	exit(1);
}
if (!policy.enabled)
	exit(0);
...
let proxy_probe_port = int(getenv('SBPROXY_ADAPTIVE_PROXY_PROBE_PORT')) ||
...
if (system(`/bin/mkdir -p ${shellQuote(RUN_DIR)} ${shellQuote('/etc/sbproxy/adaptive')}`) !== 0)
	exit(1);
loadLearned();
initFailureLog();
if (!writeRules())
