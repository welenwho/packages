...
const resources_updated = getenv('SBPROXY_RESOURCES_UPDATED') === '1';
...
const service_running = system('/etc/init.d/sbproxy running >/dev/null 2>&1') === 0;
...
const ubus = connect();
const sing_features = ubus.call('luci.sbproxy', 'singbox_get_features', {}) || {};
/* Common var end */

/* Log */
system(`mkdir -p ${RUN_DIR}`);
function log(...args) {
	const logfile = open(`${RUN_DIR}/sbproxy.log`, 'a');
	if (!logfile)
		return;
...
function restart_service(message) {
	...
	if (system('/etc/init.d/sbproxy restart >/dev/null 2>&1') !== 0) {
		log('Failed to restart SBProxy; the updated configuration has not been applied.');
		return false;
	}
