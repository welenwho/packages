...
TEST_ROOT='/tmp/homeproxy-adaptive-reverse-test'
...
mkdir -p "$UCI_DIR" "$RUN_DIR" \
	"$API_DIR/proxies/homeproxy-adaptive-final-direct-out" \
...
{"connections":[{"id":"reverse","metadata":{"host":"reverse.example","network":"tcp","destinationPort":"443"},"chains":["homeproxy-adaptive-out"],"rule":"final","download":0}]}
EOF
printf '%s\n' '{"delay":200}' >"$API_DIR/proxies/homeproxy-adaptive-final-direct-out/delay"
printf '%s\n' '{"delay":2000}' >"$API_DIR/proxies/homeproxy-adaptive-out/delay"
