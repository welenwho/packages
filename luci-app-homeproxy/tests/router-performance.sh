...
BENCH_DIR='/tmp/homeproxy-adaptive-bench'
PERF_RUN_DIR='/tmp/homeproxy-adaptive-perf'
UCI_DIR='/tmp/homeproxy-adaptive-perf-uci'
HTTP_PORT='45200'
MIXED_PORT='45201'
API_PORT='45202'
ITERATIONS='128'
...
run_case() {
	...
	/usr/bin/sing-box run -c "$config" >/tmp/homeproxy-adaptive-perf-core.log 2>&1 &
	core_pid=$!
	wait_proxy || {
		cat /tmp/homeproxy-adaptive-perf-core.log >&2
		return 1
	}

	if [ "$with_worker" = '1' ]; then
		HOMEPROXY_UCI_CONFIG_DIR="$UCI_DIR" \
		HOMEPROXY_ADAPTIVE_RUN_DIR="$PERF_RUN_DIR" \
		HOMEPROXY_ADAPTIVE_RULES_PATH="$PERF_RUN_DIR/rules.json" \
		HOMEPROXY_ADAPTIVE_LEARNED_PATH="$PERF_RUN_DIR/learned.json" \
		HOMEPROXY_ADAPTIVE_CORE_LOG_PATH="$PERF_RUN_DIR/sing-box-c.log" \
			nice -n 10 /usr/bin/ucode -S -L "$MODULE_DIR" "$WORKER" \
			>/tmp/homeproxy-adaptive-perf-worker.log 2>&1 &
		worker_pid=$!
		sleep 6
	fi
...
cp /etc/config/homeproxy "$UCI_DIR/homeproxy"
cp /tmp/homeproxy-adaptive.test "$UCI_DIR/homeproxy-adaptive"
uci -c "$UCI_DIR" -q batch <<-EOF
	set homeproxy.infra.clash_api_port='$API_PORT'
	set homeproxy-adaptive.main.enabled='1'
	set homeproxy-adaptive.main.dry_run='1'
	set homeproxy-adaptive.main.outbound='welen_urltest'
	set homeproxy-adaptive.main.poll_interval='10'
	set homeproxy-adaptive.main.max_load='0'
	commit homeproxy
	commit homeproxy-adaptive
EOF
...
/usr/sbin/uhttpd -f -D -S -N 8 -p "127.0.0.1:$HTTP_PORT" -h "$BENCH_DIR" \
	>/tmp/homeproxy-adaptive-perf-http.log 2>&1 &
http_pid=$!
