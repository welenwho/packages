...
import { resolveUrltestNodes } from 'homeproxy';
...
const nodes = resolveUrltestNodes(fakeUci, 'homeproxy', [ 'manual', 'alpha' ], [ 'sub-a' ]);
if (length(nodes) !== 3 || nodes[0] !== 'manual' || nodes[1] !== 'alpha' || nodes[2] !== 'beta')
	die(sprintf('subscription URLTest expansion failed: %.J\n', nodes));

const other = resolveUrltestNodes(fakeUci, 'homeproxy', [], [ 'sub-b' ]);
