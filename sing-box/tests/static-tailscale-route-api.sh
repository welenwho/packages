...
WORKFLOW="$PACKAGE_ROOT/../.github/scripts/core-release.py"
...
grep -Eq '^TAILSCALE_MODULE_VERSION:=v[^[:space:]]+$' "$MAKEFILE"
...
grep -Fq "upstream['draft'] or upstream['prerelease']" "$WORKFLOW"
grep -Fq "re.fullmatch(r'v\d+\.\d+\.\d+', tag)" "$WORKFLOW"
